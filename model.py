...
class LGFN(nn.Module):
    ...
    def flops(self, H, W):
        ...
        # dwconv
        flops += H * W * self.hidden_features * 3 * 3
        # conv2
        flops += H * W * self.dim * self.hidden_features
        print("LGFN:{%.2f}"%(flops/1e9))
        return flops
...
class DFCB(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer=nn.LayerNorm, act_layer=nn.GELU):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.layernorm = norm_layer(in_channels)
        # self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0)
        self.DCN1 = DeformableConv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.ac1 = act_layer()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.ac2 = act_layer()
        self.DCN2 = DeformableConv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.ac3 = act_layer()
    ...
    def flops(self, H, W):
        ...
        # DCN1
        flops += self.DCN1.flops(H, W)
        # conv
        flops += H * W * self.out_channels * self.in_channels * 3 * 3
        # DCN2
        flops += self.DCN2.flops(H, W)

        print("DFCB:{%.2f}" % (flops / 1e9))
        return flops
...
class EDCB(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=1, dilation=1, bias=True):
        ...
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, bias=bias)
        self.conv2 = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, bias=bias)

    def forward(self, x):
        feature = self.conv1(x)
        edge = torch.abs(x - F.avg_pool2d(x, kernel_size=3, stride=1, padding=1))
...
class EdgeBlock(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer=nn.LayerNorm, act_layer=nn.GELU):
        ...
        self.in_channels = in_channels
        self.layernorm = norm_layer(in_channels)
        self.edge1 = EDCB(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.ac1 = act_layer()
    ...
    def flops(self, H, W):
        ...
        flops += H * W * self.in_channels
        # edge1
        flops += self.edge1.flops(H, W)
        print("EdgeBlock:{%.2f}" % (flops / 1e9))
        return flops

#########################################
########### EGWinTransformer #############
class EGWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear',token_mlp='LGFN',
                 modulator=True,cross_modulator=False, bias=True):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(dim, win_size=to_2tuple(self.win_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, token_projection=token_projection)
        self.eca_attn = eca_layer(dim)
        self.dfcb = DFCB(in_channels=dim, out_channels=dim)
        self.edgeb = EdgeBlock(in_channels=dim, out_channels=dim)
    ...
    def forward(self, x, mask=None):
        ...
        shortcut = x
        shortcut0 = self.norm1(x)
        x = shortcut0.view(B, H, W, C)
        ...
        # DFCB
        x1 = self.dfcb(shortcut0)

        # FFN
        x = shortcut + self.drop_path(x)
        xe =shortcut + self.edgeb(shortcut)
        x = x + xe
        ...
        # eca_attn
        shortcut1 = self.norm3(x)
        x2 = self.eca_attn(shortcut1)
        x = x2 + x
        return x
    ...
    def flops(self):
        ...
        flops += self.edgeb.flops(H, W)
        # norm2
        flops += self.dim * H * W
        # mlp
        flops += self.mlp.flops(H, W)
        # dfcb
        flops += self.dfcb.flops(H, W)
        # norm3
        flops += self.dim * H * W
        # eca
        flops += self.eca_attn.flops()
        return flops


#########################################
########### Basic layer of ATHID ################
class BasicformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        if shift_flag:
            self.blocks = nn.ModuleList([
                EGWinTransformerBlock(dim=dim, input_resolution=input_resolution,
...
class ATHID(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        # Encoder
        self.encoderlayer_0 = BasicformerLayer(dim=embed_dim,
        ...
        self.encoderlayer_1 = BasicformerLayer(dim=embed_dim*2,
        ...
        self.encoderlayer_2 = BasicformerLayer(dim=embed_dim*4,
        ...
        self.encoderlayer_3 = BasicformerLayer(dim=embed_dim*8,
        ...
        # Bottleneck
        self.conv = BasicformerLayer(dim=embed_dim*16,
        ...
        self.decoderlayer_0 = BasicformerLayer(dim=embed_dim*16,
                            output_dim=embed_dim*16,
                            input_resolution=(img_size // (2 ** 3),
                                                img_size // (2 ** 3)),
                            depth=depths[5],
        ...
        self.decoderlayer_1 = BasicformerLayer(dim=embed_dim*8,
                            output_dim=embed_dim*8,
                            input_resolution=(img_size // (2 ** 2),
                                                img_size // (2 ** 2)),
                            depth=depths[6],
        ...
        self.decoderlayer_2 = BasicformerLayer(dim=embed_dim*4,
                            output_dim=embed_dim*4,
                            input_resolution=(img_size // 2,
                                                img_size // 2),
                            depth=depths[7],
        ...
        self.decoderlayer_3 = BasicformerLayer(dim=embed_dim*2,
                            output_dim=embed_dim*2,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[8],
...
if __name__ == "__main__":
    ...
    model_restoration = ATHID(img_size=input_size, embed_dim=32, depths=depths,
                 win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='LGFN', shift_flag=False)
